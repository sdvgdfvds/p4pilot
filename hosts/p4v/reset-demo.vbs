...
command = Quote("node") & " " & Quote(entry) & " reset --repo-root " & _
  Quote(repoRoot) & " --demo-root " & Quote(demoRoot)
exitCode = shell.Run(command, 0, True)

If exitCode = 0 Then
  MsgBox "Demo reset complete. No files are open.", vbInformation, "p4pilot"
Else
  MsgBox "p4pilot could not reset. See " & logPath, vbCritical, "p4pilot"
End If
