...
if ($InstallHelp) {
    if ($env:P4PILOT_ALLOW_TRIGGER_INSTALL -ne "1") {
        Write-Error "Refusing install guidance without P4PILOT_ALLOW_TRIGGER_INSTALL=1. This prevents accidental production trigger edits from a casual copy-paste."
        exit 2
    }
    @"
...
if (Test-DeniedUser -Candidate $User) {
    Write-Error "p4pilot submit deny: user '$User' is not permitted to submit (change=$changeLabel client=$clientLabel). Prepare the changelist with the agent; a human account must submit after review."
    exit 1
}
